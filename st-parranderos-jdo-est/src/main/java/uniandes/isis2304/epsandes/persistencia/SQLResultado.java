...
import uniandes.isis2304.epsandes.negocio.Resultado;
...
class SQLResultado 
{
	 ...
	 */
	public SQLResultado (PersistenciaEPSAndes pp)
	{
		this.pp = pp;
	}
	 ...
	 */
	public Resultado darResultadoPorId (PersistenceManager pm, long id) 
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + pp.darTablaResultado () + " WHERE id = ?");
		q.setResultClass(Resultado.class);
		q.setParameters(id);
		return (Resultado) q.executeUnique();
	}
	 ...
	 */
	public List<Resultado> darResultado (PersistenceManager pm)
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + pp.darTablaResultado ());
		q.setResultClass(Resultado.class);
		return (List<Resultado>) q.executeList();
	}
